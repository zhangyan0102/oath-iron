/**
 * jQuery plugin "Swipe slider".  轮播图引用的js
	...
	$.fn.swipeslider = function(options) {
		...
		function swipeEnd(event) {
			if (slidingState == 2) {
				...
				currentSlide = pixelOffset < startPixelOffset ? currentSlide + 1
						: currentSlide - 1;
		...
		function startAutoPlay() {
			if (allowSlideSwitch) {
				animationDelayID = window.setTimeout(performAutoPlay,
						autoPlayTimeout);
			}
		}
		...
		function insertBullets(count) {
			...
			for (var i = 0; i < count; i++) {
				...
				if (i == 0) {
					bulletList.append('<li class="sw-slide-' + i
							+ ' active"></li>');
				} else {
